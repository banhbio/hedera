...
def identify_potential_chimera_bins(hallmark_gene_summary, depth, scgs_list):
...
def main():
    ...
    parser.add_argument('-s', '--scgs', type=str, required=True, help='Comma-separated list of NCLDV single copy genes.')
    parser.add_argument('-d', '--depth', type=str, required=True, help="Path to the depth file (totalAvgDepth in 3rd column) in TSV format.")
    parser.add_argument('-o', '--output', type=str, required=True, help='Path to the output result TSV file.')
    args = parser.parse_args()
    
    scgs_list = args.scgs.split(',')

    df = identify_potential_chimera_bins(args.hallmark_gene_summary, args.depth ,scgs_list)
